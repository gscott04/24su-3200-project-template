...
if st.button('Get Counselor Contact Info', type='primary', use_container_width=True):
    try:
        url = f'http://api:4000/g/guardian/{camperID}'
        ...
        if response.status_code == 200:
            try:
                ...
                st.write(f"Contact info for your child {camperID}")
                if counselor_contact:
                    for item in counselor_contact:
                        st.write(f"Counselor Phone: {item['phone']}, Counselor Email: {item['email']}")
                else:
